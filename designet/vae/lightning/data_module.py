...
class GlyphDataModule(L.LightningDataModule):
    """Lightning wrapper around :class:`designet.vae.glyph_dataset.GlyphDataset`."""
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        lengths = self._split_lengths(len(dataset))
    ...
    def _has_split_csvs(self) -> bool:
        paths = (self.train_csv_path, self.val_csv_path, self.test_csv_path)
        return any(path is not None for path in paths)
    ...
    def _build_dataset(self, csv_path: Path) -> GlyphDataset:
        return GlyphDataset(
